...
```{r}
genes_of_interest = c("FMR1", "ZMYND8", "UPF1", "SPAST", "ATL1", "CTNNB1", "KCNQ2", "SPR")
exp_data = list()
datasets = c("BSpan", "GTEx", "BSeq", "PE", "HDBR")
for (x in datasets) {
  message(paste("Subsetting samples for", x))
...
vel_fmr1 = snRNA$vel %>% 
  ...
  scale_fill_manual(values=c("Control"= "#61bdbd",
                             "ASD" = "#ee6d6d")) + ggtitle("FMR1 - Velmeshev") + labs(fill = "Diagnsosi")
```
...
cam$cer = read.csv(file.path(sn.dir, "cameron/Cameron-metadata.csv"), header=TRUE, check.names = FALSE)
cam$thal =  read.csv(file.path(sn.dir, "cameron/Cameron-thal-exp.csv"), header=TRUE, check.names =FALSE)
cam$ge
cam$dlpfc
cam$hipp
```
...
```{r}
cer.thal = read.csv("/home/neuro/Documents/BrainData/single-cell/cameron/Cameron-thal-exp.csv", header=TRUE, check.names = FALSE)
head(cer.thal)[1:10]
