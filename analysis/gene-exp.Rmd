...
```{r}
ggsave(filename = here::here("output/Thesis_plots/Examples/ZIC2-zscore.svg"),
...
calc_lm = function(gene, dataset, region, md){
    ...
    exp = dataset %>% 
    as.data.frame() %>%
    dplyr::select(contains(subset$SampleID)) %>%
    rownames_to_column("EnsemblID") %>%
    left_join(gene2symbol, by = "EnsemblID") %>% 
    dplyr::filter(gene_name %in% gene) %>% 
      column_to_rownames("EnsemblID") %>% melt()
  
  f <- lm(exp ~ as.integer(md$PMI.intervals))
  return(f)
...
trial = calc_lm("RNASE2", e$Raw$GTEx, "Cerebellum", md$GTEx)
