...
md$BrainSeq = read.csv(file.path(bseq.out,"BrainSeq-metadata.csv"), header=TRUE, check.names = FALSE, row.names = 1)
md$BrainSpan = read.csv(file.path(bspan.out,"BrainSpan-metadata.csv"), header=TRUE, check.names = FALSE, row.names = 1)
md$GTEx =  read.csv(file.path(gtex.out,"GTEx-metadata.csv"), header=TRUE, check.names = FALSE, row.names =1)
md$HDBR = read.csv(file.path(hdbr.out, "HDBR-metadata.csv"), header=TRUE, check.names = FALSE, row.names =1)
#md$FANTOM5 = read.csv(file.path(dir, "Fantom5/Formatted/FANTOM5-metadata.csv"), header=TRUE, check.names = FALSE)
md$PsychEncode = read.csv(file.path(pe.out, "PsychEncode-metadata.csv"), header=TRUE, check.names = FALSE)
...
md_missing = do.call(rbind, na_calc) %>%
  rownames_to_column("Dataset") %>%
...
ggsave(filename = here::here("output/Thesis_plots/Metadata/missing_md.png"), 
       height=5, width = 6, units = "in", plot=md_missing)
#dev.off()
...
```{r}
write.csv(file = file.path(bseq.out,"BrainSeq-metadata-subset.csv"), md$BrainSeq)
write.csv(file= file.path(bspan.out,"BrainSpan/Formatted/BrainSpan-metadata-subset.csv"), md$BrainSpan)
write.csv(file = file.path(gtex.out,"GTEx/Formatted/GTEx-metadata-subset.csv"), md$GTEx)
write.csv(file= file.path(pe.out, "PsychEncode/Formatted/PsychEncode-metadata-subset.csv"), md$PsychEncode)
write.csv(file = file.path(hdbr.out, "HDBR/Formatted/HDBR-metadata-subset.csv"), md$HDBR)
```
