...
This R markdown document contains the complete code for determing drivers of variation of bulk RNA-sequencing files for Brain Integrative Transcriptome Hub (BITHub). All datasets were preprocessed using the code in `01-bulk-preprocess-data.Rmd`, important metadata were retained in `03-bulk-preprocess-data.Rmd` and deconvolution analysis was performed in `04-bulk-deconvolution.Rmd`. 
...
e$Raw$GTEx <- read.csv(file.path(gtex.out, "GTEx-exp.csv"), 
                       check.names = FALSE, row.names = 1)
e$Raw$PE <- read.csv(file.path(pe.out, "PsychEncode-exp.csv"), 
                     row.names =1, check.names = FALSE)
e$Raw$BSpan <- read.csv(file.path(bspan.out, "BrainSpan-exp.csv"),
                        check.names = FALSE, row.names = 1)
e$Raw$BSeq <- read.csv(file.path(bseq.out,"BrainSeq-exp.csv"), 
                       check.names = FALSE, row.names = 1)

e$Raw$HDBR <- read.csv(file.path(hdbr.out, "HDBR-exp.csv"), 
                       check.names = FALSE, row.names = 1)  %>% 
...
md$BSeq <- read.csv(file.path(bseq.out,"BrainSeq-metadata.csv"), 
                       check.names = FALSE) %>% 
  left_join( read.csv(file.path(bseq.out, 
                                "BrainSeq-decon.csv"), 
                      header=TRUE, row.names = 1) %>%
               rownames_to_column("SampleID"), by="SampleID")
...
To investigate the impact of biological and technical factors on gene expression, I performed an initial exploratory analysis on all bulk RNA-seq datasets grouped together via a PCA. The count matrices from all bulk RNA-seq datasets were filtered to remove lowly expressed genes. The expression cut-off was selected at 1 RPKM/TPM in at least 10% of the samples 
...
cor.bseq %>% corrplot(
        ...
        p.mat =cor.bseq.test$p,
         ...
         insig = 'label_sig', pch.col = 'black', cl.pos = 'n')
...
cor.bspan %>% corrplot(
        ...
        p.mat =cor.bspan.test$p,
         sig.level = c(0.05), pch.cex = 1.5, pch.col = 'black', cl.pos = 'n',
         insig = 'label_sig')
...
cor.hdbr %>% corrplot(
        ...
        p.mat =cor.hdbr.test$p,
         sig.level = c(0.05), pch.cex = 1.5,pch.col = 'black', cl.pos = 'n',insig = 'label_sig')
```
...
cor.pe %>% corrplot(
        ...
        p.mat =cor.pe.test$p,
         sig.level = c(0.05), pch.cex = 1.5,
          pch.col = 'black', cl.pos = 'n', insig = 'label_sig')
...
Prior to performing a variancePartition analysis, the correlation of all variables considered for varianceParition analysis was performed using a Canonical Correlation Analysis (CCA), where variables with a highly correlating score (correlation > 0.95) were dropped, as high correlation can overestimate the contribution of variation 

### BrainSeq

```{r}
form.bseq <- ~ AgeNumeric + (1|Regions) + (1|Sex) + RIN +  (1|Diagnosis) + mito_ratio + rRNA_ratio + TotalNReads + MappingRate +  (1|Period) + Neurons + Astrocytes + Oligodendrocytes + Endothelia + Microglia
...
<!-- # ```{r} -->
<!-- # fmr1_2 = var_part$GTEx %>%  -->
<!-- #   dplyr::filter(gene_name == "FMR1") %>% -->
<!-- #   dplyr::select(-gene_name) %>%  -->
<!-- #   column_to_rownames("EnsemblID") %>%  -->
<!-- #   melt()  %>%  -->
<!-- #   ggplot(aes(x=variable, y = value)) + geom_bar(stat = "identity", fill = "#F16160") + -->
<!-- #   coord_flip() + theme_bw() + ylab("Fraction Variance Explained") + xlab("") + ggtitle("FMR1 - GTEx") + -->
<!-- #      theme(axis.text.x = element_text(size = 12, family = "serif", color = "black", angle = 90), -->
<!-- #         axis.text.y = element_text(size = 15, family = "serif", color = "black"),  -->
<!-- #         axis.title = element_text(size=15, family = "serif"),  -->
<!-- #          legend.box.background = element_rect(color = "black"), -->
