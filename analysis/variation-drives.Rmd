...
pe.form <- ~ (1|Sex) + (1|Diagnosis) + AgeNumeric + (1|Ethnicity) + 
  (1|Period) + (1|DonorID) + (1|individualIDSource) + (1|Death)
...
<!-- # ```{r} -->
<!-- # form.hdbr <- ~ (1|AgeInterval) + (1|Sex) + (1|Hemisphere) + (Regions+0|SequencingBatch) + (1|SequencingBatch)+  TotalNReads + MappingRate + (1|Regions) + PMI + DonorID + IntronicRate  -->
<!-- # varPar.hdbr <- fitExtractVarPartModel(hdbr,form.hdbr ,md$HDBR) -->
<!-- # varPar.hdbr = sortCols(varPar.hdbr) -->
<!-- # plot_varPart = plotVarPart(varPar.hdbr)  -->
<!-- #  -->
...
spe.hel <- decostand(doubs.spe, method = "hellinger")
```
